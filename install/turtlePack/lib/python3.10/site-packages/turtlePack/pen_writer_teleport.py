...
class PenWriter(Node):
    ...
    def __init__(self):
        super().__init__('pen_writer_teleport')
    ...
    def teleport(self, x, y):
        reqT = TeleportAbsolute.Request()
        reqT.x = float(x)
        reqT.y = float(y)
        reqT.theta = float(1.57)
        self.teleport_client.call_async(reqT)
        self.get_logger().info(f'teleport to {x}, {y}')
